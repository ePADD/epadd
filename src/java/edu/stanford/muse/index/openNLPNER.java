...
@Deprecated public class openNLPNER {
	...
	public static final int							MIN_NAME_LENGTH			= 2, MAX_NAME_LENGTH = 100;					// can't have a single char name. max # of chars in a name. above this we'll drop it. avoids openNLPNER returning garbage like sometimes it returns a string of 200K 'a's.
	...
	{
		InputStream is = new GZIPInputStream(openNLPNER.class.getClassLoader().getResourceAsStream("locations.gz"));
	...
	public static void readLocationsWG()
	{
		InputStream is = null;
		try {
			is = new GZIPInputStream(openNLPNER.class.getClassLoader().getResourceAsStream("WG.locations.txt.gz"));
			LineNumberReader lnr = new LineNumberReader(new InputStreamReader(is, "UTF-8"));
			while (true)
			{
				String line = lnr.readLine();
	...
	public static void readLocationNamesToSuppress()
	{
		...
		try {
			InputStream is = new GZIPInputStream(openNLPNER.class.getClassLoader().getResourceAsStream(suppress_file));
			LineNumberReader lnr = new LineNumberReader(new InputStreamReader(is, "UTF-8"));
			while (true)
			{
				String line = lnr.readLine();
	...
	public synchronized static void initialize() throws ClassCastException {
		...
		log.info("Initializing openNLPNER models");
		...
		log.info("Done initializing openNLPNER model in " + Util.commatize(endTimeMillis - startTimeMillis) + "ms");
	}
	...
	private synchronized static Pair<MyTokenizer, List<Triple<String, Integer, Integer>>> parseAndGetOffsets(String documentText)
	{
		...
		// replace i18n chars with space, causes annoying openNLPNER messages + perhaps slows down openNLPNER?
		...
		Span sentenceSpans[] = sFinder.sentPosDetect(documentText); // do openNLPNER sentence by sentence -- much faster than doing the entire documentText at once
		...
		for (Span sentenceSpan : sentenceSpans)
		{
			...
			for (Triple<String, Integer, Integer> t : sentenceTriples)
			{
				...
				// we tend to see a lot of annoying [Hi Sam] or [Dear Caroline] phrases. surprising openNLPNER can't handle it already.
	...
	{
		Indexer.log.info(allTypes.size() + " openNLPNER types");
	...
	{
		Pair<MyTokenizer, List<Triple<String, Integer, Integer>>> ner_result = openNLPNER.parseAndGetOffsets(text);
	...
	public static List<Pair<String, Float>> namesFromText(String text, boolean removeCommonNames, Map<String, Integer> tokenTypeWeights, boolean normalizeByLength, int max_positional_boost) throws ClassCastException {
		NERTokenizer t = (NERTokenizer) openNLPNER.parse(text);
	...
	public static void main2(String args[]) throws IOException, ClassNotFoundException {
		...
		List<Pair<String, Float>> entities = namesFromText(text, true, openNLPNER.defaultTokenTypeWeights, normalizeByLength, 1);
    ...
    public static void testOpenNLP() {
        ...
        try {
			...
			/*
			List<Pair<String,Float>> pairs = openNLPNER.namesFromText(s);
	...
	public static void main(String[] args) {
		try {
			...
			Pair<MyTokenizer, List<Triple<String, Integer, Integer>>> o = openNLPNER.parseAndGetOffsets(text);
			...
			for (String dId : dIds) {
				...
				//System.err.println("\n---------------------------\n");
				//System.err.println(openNLPNER.retainOnlyNames(li.getContents(doc, false), Indexer.getNamesOffsets(ldoc)));
				//	System.err.println(openNLPNER.retainOnlyNames(li.getContents(doc, false)));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
